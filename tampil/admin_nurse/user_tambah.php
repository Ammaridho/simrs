...
$rsData = query($query_rsData, $koneksi) or die(error());
$row_rsData = fetch_assoc($rsData);
$totalRows_rsData = num_rows($rsData);
?>
<html>
<head>
...
<?php } while($row_rsData = fetch_assoc($rsData)); ?>
 ];
 $( "#kota" ).autocomplete({
...
$kategori = query("SELECT * FROM gol_tm ORDER BY kd_gol_tm");
while($p=fetch_array($kategori)){
...
$spesialis = query("SELECT * FROM klinikdb ORDER BY kd_klinik");
while($p=fetch_array($spesialis)){
