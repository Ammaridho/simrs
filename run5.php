...
while ($data = mysqli_fetch_array($hasil))
{
  ...
  if ($pecah[0] == "REG2")
  {
	...
	$hasil_prn = mysqli_query($query_prn);
	$data_prn = mysqli_num_rows($hasil_prn);
	$data_prn = mysqli_fetch_array($hasil_prn);
	$nama_pasien = $data_prn['nama']; 
	$sex = $data_prn['jenis_kalamin']; 
	
	//Cari data dokter
	$query_dr = "SELECT * FROM user WHERE username='$kd_dokter'";
	$hasil_dr = mysqli_query($query_dr);
	$data_dr = mysqli_num_rows($hasil_dr);
	$data_dr = mysqli_fetch_array($hasil_dr);	
	$dokter = $data_dr['nama'];
	$spesialisasi = $data_dr['unit'];
	
	//Cari data klinik
	$query_klinik = "SELECT * FROM klinikdb WHERE kd_klinik='$kd_klinik'";
	$hasil_klinik = mysqli_query($query_klinik);
	$data_klinik = mysqli_num_rows($hasil_klinik);
	$data_klinik = mysqli_fetch_array($hasil_klinik);
	...
	$hasil_reg = mysqli_query($query_reg);
